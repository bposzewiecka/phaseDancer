import os
from src.utils.text import get_prev_number, get_range
...
def recursive_input2(input_file):

    def func(wildcards):

        if wildcards.number == '000' or wildcards.number == '001':
            number = '000'
        else:
            number = get_prev_number(get_prev_number(wildcards.number))
...
# This is done using flock command to avoid the race conditions if many such commands are executed concurrently.
# Next, reads appended to output fasta file (by process that receive output of the mapper) are read by tail command with -f parameter to output reads when the file grows.
...
rule clusterng_cluster_selection_read_extraction:
    ...
    run:
        try:
            ...
            raise e
#
# Running wtdbg2 assembler on selected reads. The resulting assembly is copied into convinient place in the workflow directory structure..
#
...
#
# Reference extension of the anchor sequence. During this process new anchor sequence is created used in the next iteration of the workflow.
